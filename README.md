...
* #### 무상태 ( Stateless )
   ...
   * 한 번 연결을 한 후 응답을 하면 연결을 끊기게 되면 계속해서 로그인을 무한 반복을 해야만 한다.
   * [비연결성, 무상태]
...
* #### 세션과 쿠키
   ...
   * HTTP프로토콜의 비연결성의 단점해결
   * 공통적으로 쿠키와 세션은 클라이언트와 서버의 연결을 유지해주는 역할
   * 쿠키의 경우는 클라이언트에서 연결 정보를 관리하지만 세션은 서버에서 연결 정보를 관리
   * 세션의 삭제는 저장된 속성이 더 이상 사용할 필요가 없어질 경우로 로그아웃 또는 회원 탈퇴에 이용
